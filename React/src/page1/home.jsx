...
function Home() {
    ...
    const handleLogout = async () => {
        await logout(); // Context의 logout 함수가 API 호출 및 상태 정리를 대신함
        navigate('/login');
    };
    ...
    // 5. 비로그인 시 처리 (AuthProvider에서 처리하지만 안전장치로 추가)
    if (!user) {
        navigate('/login');
        return null;
    }
    ...
    return (
        <div className='flex min-h-screen bg-[#E9ECEF] font-sans p-4 gap-4'>
            ...
            <main className='flex-1 p-10 overflow-y-auto bg-white rounded-[32px] shadow-xl relative'>
                ...
                {activeMenu === 'attendance' && <div className="bg-[#F8F9FA] p-8 rounded-[32px]"><AttendancePanel/></div>}
                {activeMenu === 'profile' && (
